...
saved_root = f"./data/{phase}_median_frame/"
new_gts_path = os.path.join(saved_root, f"{phase}_median_frame.csv")
...
for idx, row in original_gts_df.iterrows():
    ...
    total_frames = vid_cap.get(cv2.CAP_PROP_POS_MSEC) * vid_cap.get(cv2.CAP_PROP_FPS)
    vid_cap.set(cv2.CAP_PROP_POS_FRAMES, total_frames // 2)
    status, median_frame = vid_cap.read()

    # face detection
    rate = 3
    small_frame = cv2.resize(
        median_frame, (0, 0), fx=round(1 / rate, 2), fy=round(1 / rate, 2)
    )
    norm_img = np.zeros((small_frame.shape[0], small_frame.shape[1]))
    norm_small_frame = cv2.normalize(small_frame, norm_img, 0, 255, cv2.NORM_MINMAX)
    ...
    if largest_face_coords is not None:
        ...
        face_region = median_frame[Y1:Y2, X1:X2]
        img_path = os.path.join(
            saved_root,
            "images",
            os.path.basename(video_path).replace(".mp4", "") + "-0.jpg",
        )
        # save label
